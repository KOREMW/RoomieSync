...
| 가사 | 멤버 순서대로 **자동 배정** · 주기(매일/매주/매월/한 번) · **난이도** 설정 · 완료 즉시 다음 멤버로 회전(잘못 누르면 "취소"로 되돌리기) | ![Chore](docs/screens/02_chore.png) |
...
추가 차별 기능:
- **정산 자동화** — 채무 단순화로 송금 횟수 최소화 + 토스 송금 딥링크/계좌 복사 + **정산 히스토리**
...
- **로컬 알림 6종** — 오전 당번 · 저녁 미완료 · 룸메 완료 · 지출 입력 · 월말 정산 · 새 공지 (알림에서 바로 완료/교대)
- **클라우드 동기화** — Firebase Firestore(익명 인증)로 같은 그룹 멤버 간 기기 동기화
...
| 알림 | UserNotifications (Local) | APNs 인증서 불필요, 6 시나리오 모두 로컬로 처리 + 알림 액션 |
